...
- (void)autotrackPage:(UIViewController *)controller
                alias:(NSString *)alias NS_EXTENSION_UNAVAILABLE("AutotrackPage is not supported for iOS extensions.");
...
/// 设置埋点通用属性
/// @param props 事件通用属性，相同字段的新值将覆盖旧值
- (void)setGeneralProps:(NSDictionary<NSString *, NSString *> *)props DEPRECATED_MSG_ATTRIBUTE("Use +[GrowingAutotracker setGeneralProps:] instead.");

/// 清除指定字段的埋点通用属性
/// @param keys 通用属性指定字段
- (void)removeGeneralProps:(NSArray<NSString *> *)keys DEPRECATED_MSG_ATTRIBUTE("Use +[GrowingAutotracker removeGeneralProps:] instead.");
...
@property (nonatomic, assign) GrowingIgnorePolicy growingViewIgnorePolicy NS_EXTENSION_UNAVAILABLE(
    "Property growingViewIgnorePolicy is not supported for iOS extensions.");

// 手动标识该view的取值，请在该view被初始化后立刻赋值
@property (nonatomic, copy) NSString *growingViewCustomContent NS_EXTENSION_UNAVAILABLE(
    "Property growingViewCustomContent is not supported for iOS extensions.");

// 手动标识该view的tag
// 这个tag必须是全局唯一的，在代码结构改变时也请保持不变
@property (nonatomic, copy) NSString *growingUniqueTag NS_EXTENSION_UNAVAILABLE(
    "Property growingUniqueTag is not supported for iOS extensions.");
