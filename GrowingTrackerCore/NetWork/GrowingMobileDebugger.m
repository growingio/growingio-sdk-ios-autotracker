...
- (void)stop {
    ...
    [self sendJson:dict];
    self.statusWindow.statusLable.text = @"正在关闭";
    self.statusWindow.statusLable.textAlignment = NSTextAlignmentCenter;
             ...
             if ([msg isEqualToString:@"logger_open"]) {
                [GrowingWSLogger sharedInstance].loggerBlock = ^(NSArray * logMessageArray) {
                       if (logMessageArray.count > 0) {
                            ...
                            cacheDic[@"sdkVersion"] = [GrowingDeviceInfo currentDeviceInfo].appFullVersion;
                            cacheDic[@"data"] = logMessageArray;
                            [self sendJson:cacheDic.copy];
                        }
                    };
