    ...
    dispatch_once(&onceToken, ^{
        if (@available(iOS 13.0, *)) {
            ...
            if (method) {
                ...
                @throw [NSException
                    exceptionWithName:NSInternalInconsistencyException
                               reason:[NSString
                                          stringWithFormat:
                                              @"在iOS13以上，请在%@实例中实现scene:openURLContexts:以适配UrlScheme",
            ...
            if (method) {
                ...
                method_setImplementation(
                    method,
                    imp_implementationWithBlock(^(id target, UIScene *scene, NSUserActivity *userActivity) {
                ...
                @throw [NSException
                    exceptionWithName:NSInternalInconsistencyException
                               reason:[NSString
                                          stringWithFormat:
                                              @"在iOS13以上，请在%@实例中实现scene:continueUserActivity:以适配DeepLink",
                                              NSStringFromClass(delegateClass)]
                             userInfo:nil];
            }
            // 冷启动
            ...
            if (method) {
                ...
                @throw [NSException
                    exceptionWithName:NSInternalInconsistencyException
                               reason:[NSString
                                          stringWithFormat:@"在iOS13以上，请在%@实例中实现scene:willConnectToSession:"
                                                           @"options:以适配冷启动圈选、冷启动DeepLink等场景",
                                                           NSStringFromClass(delegateClass)]
                             userInfo:nil];
            }
        }
